...
species.priority <- function(gene.list, orthologue.selection.directory, download.directory, species_priority_table = "reemplazos.txt")
  { 
  ...
  setwd(orthologue.selection.directory)
  write.csv(especies.sirven, file = "replacement_set.txt", row.names = FALSE) # this could be depleted and directly use species.sirven
  ...
  for (gen.index in 1:nrow(spp.ok))
  {
    ...
    if (nrow(salida) >= 14) {
      ...
      write.table(salida, file = gen.2, quote = FALSE, col.names = FALSE, row.names = FALSE)
  ...
  write.table(absent.orthologues, file = "Not enough orthologues.txt", quote = FALSE, col.names = FALSE, row.names = FALSE)
  write.table(enough.orthologues,  file = "Enough orthologues.txt", quote = FALSE, col.names = FALSE, row.names = FALSE)
