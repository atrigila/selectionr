...
#   for (potencial.posicion in potenciales.posiciones) {
#bloques.gaps <- nrow(inicios.y.fines) #cantidad de bloques con gaps
#evaluar si para cada bloque de gap, la potencial posicion se vuelve el numero correcto
#   if (inicios.y.fines$inicio[1] > potencial.posicion) {
# if(potencial.posicion == new.number) {
#original.number <- potencial.posicion
#original.letter <- original.replaced.masked[potencial.posicion]
...
#}
#for (bloque.gap in 1:bloques.gaps) {
#  if (potencial.posicion-inicios.y.fines$length[bloque.gap] == new.number){ #cual de todas las potenciales te da el la misma posicion si no tuviese gaps
    #save potencial.posicion
#    original.number <- potencial.posicion
#    original.letter <- original.replaced.masked[potencial.posicion]
#  } else {
#    sum.gaps.blocks <- sum(inicios.y.fines$length[1:bloque.gap])
#    if (potencial.posicion-sum.gaps.blocks == new.number) {
#      original.number <- potencial.posicion
#      original.letter <- original.replaced.masked[potencial.posicion]
#    }
#  }
